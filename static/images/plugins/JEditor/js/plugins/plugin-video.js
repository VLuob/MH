(function($){
	...
	E.createMenu(function(check){
		...
		var menu = new E.Menu({
			...
			$domNormal: $('<a href="#" tabindex="-1"><i class="jeditor-menu-icon-play"></i></a>'),
			$domSelected: $('<a href="#" tabindex="-1" class="selected"><i class="jeditor-menu-icon-play"></i></a>'),
			$dot: $dot
		});
		...
		function analysisUrl(url){
			...
			switch(sourceName){
				case source.youku.name:
					var startIndex = url.indexOf('\/id_') + '\/id_'.length ;
					var endIndex = url.indexOf('.html');
					var video_id = url.substring(startIndex,endIndex);
				...
				case source.tudou.name:
					...
					var subUrl = url.substring(startIndex,endIndex);
					var codeArr = subUrl.split('/');
					var code = codeArr[3];
					var lcode = codeArr[2];
    ...
    E.plugin(function () {
        ...
        fns.onload || (fns.onload = function (resultText, xhr) {
            ...
            if (resultText.indexOf('error|') === 0) {
                ...
                var resultText = resultJson.data.fileUrl;
    ...
    E.plugin(function () {
        ...
        var $dotProgress = $('<div class="upload-progress"></div>');
        var $dotPercentage = $('<div class="percentage" style="width:30%"></div>');
        var $dotProgressLabel = $('<div class="progress-label">30%</div>');
        var $dot = editor.menus['video'].$dot;

        // if($dot){
        // 	$dotProgress.append($dotPercentage);
        // 	$dot.append($dotProgress,$dotProgressLabel);
        // }
        ...
        function render() {
            ...
            $uploadContentVideo.append($progressContainer,$uploadCompleteContainer);
        }
        ...
        $btnCompleteInsert.click(function(event) {
        	if(currVideoUrl){
        		var html = '<iframe src="' + currVideoUrl + '" style="width:640px; height:498px; max-width:100%;" frameborder=0 allowfullscreen/>';
        		editor.command(null, 'insertHtml', html);
        		$uploadContentVideo.removeClass('complete');
	...
	E.plugin(function(){
		...
		$uploadContent.append($uploadIconBtn);
