...
export default {
	...
	confirm(option,confirm,cancel, conformName, cancelName) {
		let message = typeof option === 'string' ? option : (option.title || '')
		let description = typeof option === 'string' ? '' : (option.content ? `<p>${option.content}</p>` : '')
        let content = document.createElement('div')
		...
		content.innerHTML = `<span class="m1-confirm-close" data-m1-cover-cancel></span>
					               ...
					               <div class="m1-confirm-footer">
					                 <button class="m1-btn" data-m1-cover-cancel>${cancelName || "取消"}</button>
					                 <button class="m1-btn m1-btn-primary" data-m1-cover-confirm>${conformName || "确认"}</button>
					               </div>`
