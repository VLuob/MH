...
export default class Shots extends Component {
    static async getInitialProps(ctx) {
        ...
        const { compositionStore, accountStore, adStore, globalStore } = mobxStore
        const { shotsData, fetchGetClassifications, fetchGetClientShotsList } = compositionStore
        ...
        if(req && req.headers) {
            ...
            //     if (query.follow === '') {
            //         if (!token) {
            //             res.writeHead(307, {Location: `/signin?c=${req.originalUrl || req.url}`})
            //             res.end()
            //         }
            ...
            //         sort = [{ key: 'degree', value: 'desc' }]
            //     } else if (query.like === '') {
            //         sort = [{ key: 'favors', value: 'desc' }]
            //     } else if (query.newest === '') {
            //         sort = [{ key: 'gmtPublish', value: 'desc' }]
            //     } else if (query.read === '') {
            //         sort = [{ key: 'views', value: 'desc' }]
            //     } else if (query.comment === '') {
            //         sort = [{ key: 'comments', value: 'desc' }]
            //     } else if (query.collection === '') {
            //         sort = [{ key: 'collections', value: 'desc' }]
            //     } else {
            //         terms.recommended = true
            //     }
            // }
            ...
            // 获取作品列表 
            const shotParam = { host, terms: newTerms, client: clientCode, token }
            let resultShotsAds = {}
            let resultShotData = {}
            if (terms.recommended) {
                const shotsAdParam = { host, page_code: `f_w_s`, field_code: JSON.stringify(['f_w_s_1','f_w_s_2','f_w_s_3']) }
                resultShotsAds = await adStore.fetchAdvertisement(shotsAdParam) 
                shotParam.mergeAds = resultShotsAds
                const resShotData = await fetchGetClientShotsList(shotParam)
            } else {
                resultShotData = await fetchGetClientShotsList(shotParam)
    ...
    getTitleName() {
        ...
        const scopeName = commonSortTypeMap[query.sort || CommonSortType.RECOMMEND]
