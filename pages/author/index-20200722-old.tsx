...
export default class Author extends Component {
    static async getInitialProps(ctx) {
        ...
        const { fetchAuthorList } = authorStore
        ...
        if(req && req.headers) {
            ...
            const ipParam = { host, ip }
            const ipAdr = await fetchGetIpAddress(ipParam)
            updateIpAddress(ipAdr)

            // !condition.term && (condition.term = {})
            // if(ipAdr.city) {
            //     condition.term && (condition.term.city = [ipAdr.city])
            // } else if(ipAdr.province) {
            //     condition.term && (condition.term.province = [ipAdr.province])
            // }
            ...
            // 获取创作者列表 
            const authorParam: any = { host, condition: newCondition }
            // console.log('newCondition', newCondition)
            const authorListResultData: any = await fetchAuthorList(authorParam)
