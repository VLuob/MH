...
import { Modal, Button, Icon, message, Spin } from 'antd'
...
// import Loading from '@components/features/PartLoading';
...
@observer
export default class CardIcon extends Component<Props> {
  ...
  initQiniuToken() {
    const { globalStore, qiniutoken, noReqQiniu } = this.props
    if (!qiniutoken && !noReqQiniu) {
  ...
  toCreateCard() {
    ...
    const { imageHash, imageUrl } = this.state
    if (imageUrl) {
      this.handleModalVisible(true)
      if (onClick) onClick({url: imageUrl, hash: imageHash})
    } else {

      this.uploadImage((hash) => {
        const url = `${config.RESOURCE_QINIU_DOMAIN}/${hash}`
        this.setState({imageHash: hash, imageUrl: url})
        this.handleModalVisible(true)
        // window.open(`${config.RESOURCE_QINIU_DOMAIN}/${imageHash}?attname=${cardName}.png`)
        if (onClick) onClick({url, hash})
      })
    }
  }
  ...
  uploadImage(fn) {
    ...
    if (imageHash) {
      ...
      console.log('card build start')
      this.setState({isUploading: true})
      this.getImageUrl(imgUrl => {
        console.log('card build end')
        const { qiniuToken, cardName } = this.props;
        const token = qiniuToken;
        console.log('card upload start')
        helper.qiniuPutb64({base64: imgUrl, token}).then((res) => {
          console.log('card upload end')
          const imageUrl = `${config.RESOURCE_QINIU_DOMAIN}/${res.hash}`
          this.setState({imageUrl, imageHash: res.hash, isUploading: false})
          if (fn) fn(res.hash)
        }).catch(() => {
          message.error('图片生成出错，请重试')
  ...
  handleShareWeibo = () => {
   ...
   this.uploadImage((imageHash) => {
      const cover = `${config.RESOURCE_QINIU_DOMAIN}/${imageHash}`
  ...
  handleShareWechat = () => {
    this.uploadImage((imageHash) => {
      const wechatUrl = `${location.origin}/card/${imageHash}`
      this.setState({imageUrl: `${config.RESOURCE_QINIU_DOMAIN}/${imageHash}`, wechatUrl})
  ...
  render() {
    const { cardName, type, item, children, ...rest } = this.props
    const { showShareIcons, showQrcode, imageUrl, wechatUrl, modalVisible, isUploading, iframeVisible } = this.state

    const weiboUrl= this.getWeboUrl()
    ...
    return (
      <>
      {!children 
      ? <CIcon name="card" onClick={this.handleCreateCard} />
      ...
      {modalVisible && <Modal
        ...
        onCancel={e => this.handleModalVisible(false)}
        footer={null}
      >
        <div className="modal-card-container">
          ...
          <div className="card-footer">
            ...
            <div className="card-btns">
                <Button href={`${imageUrl}?attname=${cardName}.png`} target="_blank">保存</Button>
      ...
      <Frame 
        ...
        className="share-card-iframe"
        frameBorder={0}
        initialContent={`<!DOCTYPE html><html><head><style>*{margin:0;padding:0;border:0;outline:0} body{font-family: "Microsoft YaHei",微软雅黑;overflow:hidden;} div{box-sizing: border-box;}</style></head><body><div id="mountHere"></div></body></html>`}
        mountTarget='#mountHere'
      >
        <div>{cardContent}</div>
      </Frame>}
      <Portal>
        {isUploading && <div className="card-create-loading">
          <div>
            <Spin />
            <div className="text">正在生成卡片...</div>
          </div>
        </div>}
      </Portal>
      </>
