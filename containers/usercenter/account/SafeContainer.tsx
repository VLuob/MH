...
@inject(stores => {
    ...
    const { GTVInfo, phoneInfo, userClientInfo, fetchEmailBind, fetchPhoneBind, isEmailNextStep, 
        isPhoneNextStep, fetchPhoneVeriBind, fetchGTVerifyCode, fetchNewGTVerifyCode, resetGTVInfo, fetchChangePassword } = accountStore
...
export default class SafeContainer extends Component {
    ...
    async componentDidMount() {
        const { fetchGTVerifyCode, resetGTVInfo } = this.props
        resetGTVInfo()
        fetchGTVerifyCode()
    ...
    handlePhoneOk = e => {
        const { phoneInfo, fetchPhoneVeriBind } = this.props
        ...
        if(!!gtResult && gtResult.geetest_challenge && gtResult.geetest_seccode && gtResult.geetest_validate) {
            const param = { phone: phoneVal, code: verifyVal, verify_token: token || phoneInfo.token }
            fetchPhoneVeriBind(param).then(res => {
                message.destroy()
                if (res.success) {
    ...
    render() {
        const { title, visible, emailVal, pwdVal, newPwdVal, phoneVal, verifyVal, isDownCount, downCount, pwdVisible, phoneVisible } = this.state
        const { 
            GTVInfo, 
            isEmailNextStep, 
            userClientInfo,
        } = this.props
        const { email, mobilePhone, phone, mobileBind } = userClientInfo

        return (
            <div className='safe-container'>
                <ul className='safe-list'>
                    <li>
                        <span className='name'>邮箱: &nbsp;&nbsp; {email}</span>
                        <Button type='primary' onClick={this.showEmailModal}>修改</Button>
                        <span className='verify'><Icon type='check-circle' theme='filled' /> 已验证</span>
                    </li>
                    <li>
                        <span className='name'>手机: &nbsp;&nbsp; {phone || mobilePhone}</span>
                ...
                <Modal
                    ...
                    <Row>
                        <Col>
                            ...
                            <a href='/password' className='link' target="_blank">忘记密码？</a>
                        </Col>
                        <Col>
                ...
                >
                    <Row>
                        <Col>
                            <Input type='text' value={phoneVal} placeholder={`请输入新手机号`} className='new-border-input' onChange={this.handlePhoneChange} />
                        </Col>
                        <Col>
                            {!GTVInfo.success &&
                        ...
                        <Col>
                            <Input type='text' value={verifyVal} placeholder={`请输入验证码`} className='new-verify-input' onChange={this.handleVerifyChange} />
