...
const COOKIE_HIDE_TIPS_BOX = 'author_banner_set_hide_tip'
...
@inject(stores => {
    const { userCenterStore, authorStore, globalStore } = stores.store
    const { briefInfo } = authorStore
...
@observer
export default class AuthorBannerModify extends Component {
  state = {
    visible: false,
    tabKey: 'default',
    currentImage: '',
  ...
  initData() {
    ...
    const hideTips = jsCookie.get(COOKIE_HIDE_TIPS_BOX)
    if (!hideTips) {
  ...
  handleOk = () => {
    const { authorStore, briefInfo } = this.props
    ...
    authorStore.modifyAuthorBanner({
      isDefault,
      type: briefInfo.type,
      ...
      org_id: briefInfo.id,
        ...
        if (res.success) {
          ...
          message.success(isDefault ? '修改成功' : '修改已提交，审核通过后自动更新')
        } else {
  ...
  handleCloseTip = () => {
    ...
    jsCookie.set(COOKIE_HIDE_TIPS_BOX, '1', options)
  }
  ...
  render() {
    const { authorBanners, briefInfo } = this.props
    const { 
        visible, 
        tabKey,
        currentImage,
    ...
    // console.log(toJS(briefInfo))
    const editionType = briefInfo.editionType || EditionType.FREE
    const isFreeEdition =  editionType === EditionType.FREE

    return (
      <>
        <Button type='primary' className='banner-change' onClick={this.bannerChange}>
          <span>更换背景</span>
          {showTip && <div className="tips-box" onClick={this.clickTips}>
            <span className="tips-text">创作者主页可以上传自定义封面图片了！</span>
            <span className="close" onClick={this.handleCloseTip}>×</span>
          </div>}
        </Button>
        <Modal
            visible={visible}
