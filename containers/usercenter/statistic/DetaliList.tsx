...
export default class DetailList extends PureComponent {
  ...
  render() {
    const { data, onPagination, compositionType } = this.props
    const list = data.list || []
    const page = data.page
    const size = data.size
    const total = data.total
    if (compositionType === CompositionTypes.ARTICLE) {
      this.columns[0].title = '文章'
      this.columns[this.columns.length - 1].title = '文章排行'
    } else {
      this.columns[0].title = '作品'
      this.columns[this.columns.length - 1].title = '作品排行'
    }
