...
@inject(stores => {
  const { commentStore, accountStore, globalStore } = stores.store
...
export default class CommentBox extends Component {
  ...
  handleSubmit = ({content, parentId, replyId, gtResult, callback}) => {
    ...
    const { commentStore, compositionId, currentUser, type, ip} = this.props
    const client_code = jsCookie.get(config.COOKIE_MEIHUA_CLIENT_CODE)
    ...
    return (
      ...
      <div className="comment-box">
        ...
        <div className="comment-header">
          ...
          {!isLogin && <span className="signout-tip-text">点击 <a onClick={this.handleLogin}>登录</a> 梅花网，用您的头像和昵称秀出您的观点</span>}
        </div>
        {!hideFirstForm && <CommentForm
          isLogin={isLogin}
          placeholder={isLogin ? '发表评论...' : '可免登录评论，未登录状态的评论会经过编辑审核后显示，全站可见，请文明发言。'}
