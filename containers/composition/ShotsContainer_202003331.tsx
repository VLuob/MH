    ...
    return new Promise((resolve, reject) => {
      ...
      getVideoMeta(file).then((meta: any) => {
        ...
        // } else if (edition.editionType === EditionType.FREE && (meta.width >= 1920 && meta.height >= 1080 || meta.width >= 1080 && meta.height >= 1920)) {
          ...
          Modal.confirm({
            title: '温馨提示',
            content: `您的创作者版本不支持${powerTip}分辨率的视频，您可以选择在线升级或联系销售顾问`,
            cancelText: '返回修改',
            okText: '在线升级',
            okButtonProps: {className: 'themes'},
            onOk: () => {
        ...
        //   && (meta.width >= 3840 && meta.height >= 2160 || meta.width >= 2160 && meta.height >= 3840)) {
          ...
          Modal.confirm({
            title: '温馨提示',
            content: '您的创作者版本不支持4K分辨率的视频，您可以选择在线升级或联系销售顾问',
            cancelText: '返回修改',
            okText: '在线升级',
            okButtonProps: {className: 'themes'},
            onOk: () => {
    ...
    return (
      ...
      <div className="shots-edit-top">
        ...
        {authorId && <div className="top-item top-right">
          {!isAdvancedEdition && <span>本月剩余发布量：{authorItem.monthLeftPublishedQuantity || 0} 条，账户累积剩余发布量：{authorItem.totalLeftPublishedQuantity || 0} 条，</span>}<a href={creationUrl} target="_blank" style={{color: '#168dd7'}}>发布作品管理</a>
        </div>}
      </div>
