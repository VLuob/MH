...
export default class ShotsContainer extends Component {
  ...
  componentDidMount() {
    ...
    // session.set('publish_author_id', 1)
  }
  ...
  handleSubmit = (e) => {
    ...
    if (!isAgree) {
      ...
      form.validateFieldsAndScroll((err, values) => {
        if (err || !this.checkUploadingWorks()) {
          return false
        }
  ...
  handlePublish(values) {
    const { query, form, currentUser = {}, detail = {}, compositionStore } = this.props
    const { isSyncMawards, tags, brandItem = {} } = this.state
    ...
    this.submitSave(params, (res) => {
      const { authorId, edition, authorItem } = this.getCurrentAuthorEdition()
      ...
      if (res.success) {
        // 保存成功后清理session作者id信息
        session.remove(SESSION_SELECT_AUTHOR_ID)

        const compositionId = query.id || res.data
        param.id = compositionId
        param.code = 'S100000'
        const jumpUrl = `/composition/shots/result?${qs.stringify(param)}`

        if (isSyncMawards) {
          const files = this.arragementValidFiles()

          let filesHtml = ''
          files.forEach(item => {
            if (item.type === UploadFileTypes.WORKS_IMAGE) {
              filesHtml += `<p><img src="${item.url}" alt="${item.title}" /></p>`
            } else {
              filesHtml += `<p><video src="${item.url}" /></p>`
            }
          })

          const detailUrl = `${config.CURRENT_DOMAIN}/shots/${compositionId}`
          const mawarksAuthorInfo = {
            worksID: query.id,
            title: params.title,
            caseUser: brandItem.chName || detail.brandName,
            logo: params.cover,
            content: `<h2>${params.title}<h2>
            <p>${params.summary}</p>
            ${filesHtml}
            <p><a href="${detailUrl}" target="_blank">${detailUrl}</a></p>`,
            tags,
            authCode: authorItem.code,
            categoryID: params.categoryId,
            formID: params.formId,
          }
          const userName = currentUser.username
          const authCode = md5(`${userName}Mawards@2020`)
          const mawarksParam = {
            authorInfo: JSON.stringify(mawarksAuthorInfo),
            authCode,
            userName,
          }

          // console.log(JSON.stringify(mawarksAuthorInfo))

          axios({
            method: 'post',
            url: 'https://mawards_api.meihua.info/api/works/create',
            headers: {
            ...
            data: JSON.stringify(mawarksAuthorInfo),
          }).then((response) => {
            ...
            if (res.success) {
              ...
              message.error(res.data || '同步至梅花创新奖失败')
              ...
              delete urlParam.id
              Router.pushRoute(`/shots/edit/${compositionId}?${qs.stringify(urlParam)}`)
            }
          })
            .catch((error) => {
              this.setState({ submitting: false })
              message.error('同步至梅花创新奖失败')
              const orgId = authorId
              const urlParam = {
                ...query,
                orgId,
          ...
          // compositionStore.fetchSyncMawards(mawarksParam).then(res => {
          //   this.setState({submitting: false})
          //   if (res.success) {
          //     location.href = jumpUrl
          //   } else {
        ...
        if (res.success) {
          ...
          compositionStore.fetchCompositionPreviewCode({ compositionId }, (res) => {
            if (res.success) {
              ...
              delete orgParam.id
              Router.pushRoute(`/shots/edit/${compositionId}?${qs.stringify(orgParam)}`)
            }
          })
        } else {
          message.error(res.data.msg)
  ...
  handleSyncMawardsChange = e => {
    const { isSyncMawards, tags } = this.state
    const mawardsTagName = '2020年梅花创新奖参赛作品'
    let newTags = [...tags]
    if (!isSyncMawards) {
      if (!tags.includes(mawardsTagName)) {
        newTags.push(mawardsTagName)
      }
    } else {
      newTags = newTags.filter(v => v !== mawardsTagName)
    }
    this.setState({ isSyncMawards: !isSyncMawards, tags: newTags })
  }
        ...
        <div className="shots-edit-top">
          <div className="shots-publish-menu-box">
            <div className="publish-menu-item active">
              <a className="btn-normal"><CustomIcon name="upload" /> <span className="text">发布作品</span></a>
              <Popover
                placement="right"
            ...
            <div className="publish-menu-item">
              <a className="btn-normal" href={creationUrl} target="_blank"><CustomIcon name="management" /> <span className="text">作品管理</span></a>
              <Popover
                placement="right"
                content={<span className="text">作品管理</span>}
              >
                <a className="btn-mini" href={creationUrl} target="_blank"><CustomIcon name="management" /></a>
              </Popover>
            </div>
            <div className="publish-menu-item">
              <a className="btn-normal" href={statisticsUrl} target="_blank"><CustomIcon name="statistics" /> <span className="text">作品统计</span></a>
              <Popover
                placement="right"
                content={<span className="text">作品统计</span>}
              >
                ...
                <div className="shots-upload-icons">
                  ...
                  <div className="upload-icon-item">
                    ...
                    <div className="upload-icon-info">
                      ...
                      <div className="info-desc">视频分辨率720P以上，视频大小不大于1G</div>
        ...
        <div className="shots-edit-bottom">
          <div className="bottom-handle-wrapper">
            ...
            <div className="bottom-handle-list">
              ...
              <div className="bottom-handle-item">
                <Checkbox checked={isSyncMawards} onClick={this.handleSyncMawardsChange}>预约报名</Checkbox> <a href="https://mawards.meihua.info" className="copyright-link" target="_blank">《2020梅花创新奖》</a>
              </div>
            </div>
          </div>
