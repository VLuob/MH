...
const defaultGalleryHeight = 394
...
export default class Gallery extends Component<Props, State> {
  ...
  initSwiter() {
    const _this = this
    const { index, autoPlay, galleryId } = this.props
    // const slidesPerView = window.outerWidth <= 1360 ? 7 : 9
    const galleryThumbs = new Swiper(`.gallery-thumbs-pc-${galleryId}`, {
      spaceBetween: 10,
    ...
    this.mySwiper = new Swiper(`.gallery-swiper-pc-${galleryId}`, {
  ...
  transitionEnd = (currSwiper) => {
    ...
    if (!this.scrollFirstEnter) {
      if (isFullscreen) {
        ...
        // const rect = mySwiper.el.getBoundingClientRect()
        const offsetTop = mySwiper.el.offsetTop
  ...
  render() {
    const { files, filesCount,  className, detail, author={}, isHorizontalScreen, galleryId, index  } = this.props
    const { 
      isFullscreen,
      hideThumbs,
      currentIndex,
