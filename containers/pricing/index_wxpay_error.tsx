...
import { EditionScope, EditionType, AddedServiceType, AuthorType, PayType } from '@base/enums'
...
export default class PricingContainer extends Component {
  ...
  state = {
    ...
    websiteSelectAuthorVisible: false,
  }

  // static getDerivedStateFromProps(nextProps, prevState) {
  //   const nextScope = Number(nextProps.query.scope)
  //   const nextEdition = Number(nextProps.query.v)
  //   let nextState = {}
  //   if (nextScope && nextScope !== prevState.pricingScope) {
  //     nextState.pricingScope = nextScope
  //   }
  //   if (nextEdition && nextEdition !== prevState.edition) {
  //     nextState.edition = nextEdition
  //   }
  //   return nextState
  // }
  ...
  checkAuthorSelect() {
    const { authorId, pricingScope, edition } = this.state
    const { authors } = this.props
    const authorItem = (authors || []).find(item => item.id === authorId) 
    const authorEdition = authorItem.edition || {}

    if (this.isAdvancedAuthorBuyStandardEdition(authorId)) {
  ...
  // 检测高级版创作者购买标准版本
  isAdvancedAuthorBuyStandardEdition(authorId) {
    const { pricingScope, edition } = this.state
    const { authors } = this.props
    const authorItem = (authors || []).find(item => item.id === authorId) || {}
    const authorEdition = authorItem.edition || {}
    if (pricingScope === EditionScope.EDITION_PACKAGE) {
  ...
  handleAuthorSelect = (authorId, option) => {
    ...
    //   this.requestFee()
    // })
    this.setAuthorSelect(authorItem, authorItem)
  }

  setAuthorSelect(authorId, authorItem) {
    const { pricingScope } = this.state
    const extraService = (authorItem || {}).extraService || []
    const addedTypes = extraService.map(item => item.editionType)
    const nextState = {authorId, authorItem}
  ...
  handleAlipay = () => {
   ...
   handleUnionpay = async () => {
    ...
    if (response.success) {
      ...
      window.location.href = `/pricing/pay/result?${fields.scope}`
    }
  }

  requestWxpayQrcode = async () => {
    const { payWay } = this.state
    if (payWay !== PayType.WXPAY) {
      return
    }
    const res = await this.createOrderExtendDescription()
    if (res.success) {
  ...
  handleSelectAuthorConfirm = (authorItem) => {
    ...
    if (this.isAdvancedAuthorBuyStandardEdition(authorId)) {
      ...
      this.setAuthorSelect(authorId, authorItem)
      this.handleSelectAuthorVisible(false)
    }
  }
  ...
  handleShotsWebsite = (e) => {
    ...
    if (allAuthors.length > 0) {
      this.handleWebsiteSelectAuthorVisible(true)
    } else {
  ...
  handleWebsiteSelectAuthorVisible = (flag) => {
    this.setState({websiteSelectAuthorVisible: !!flag})
  }

  handleWebsiteSelectAuthorConfirm = (authorItem) => {
    const authorId = (authorItem || {}).id
    if (authorId) {
  ...
  render() {
    ...
    const visibleSubmitBtn = payWay !== PayType.WXPAY
    ...
    return (
      <div className="pricing-body-bg">
          ...
          {showPayContent && <div className="pricing-section-pay" id="form">
            ...
            <div className="pricting-pay-bottom">
                ...
                <div className="pricing-pay-bottom-item bottom-right">
                  ...
                  {visibleSubmitBtn && <Button type="primary" onClick={this.handleSubmitClick}>{btnSubmitText}</Button>}
                </div>
            </div>
            <div className="pricing-pay-wxpay-qrcode">
              <WxpayQrcode 
              />
            </div>
          </div>}
        ...
        />}
        {websiteSelectAuthorVisible && <SelectAuthorModal
          visible={websiteSelectAuthorVisible}
          authorId={(authors[0] || {}).id}
          authors={authors}
          onCancel={e => this.handleWebsiteSelectAuthorVisible(false)}
          onConfirm={this.handleWebsiteSelectAuthorConfirm}
        />}
      </div>
    )
  }
